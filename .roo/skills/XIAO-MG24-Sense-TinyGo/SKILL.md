...
description: >
  ...
  TinyGo, Silicon Labs, ARM Cortex-M33, Zigbee, Thread, Matter, BLE 5.3, IMU, microphone, pinout,
  GPIO, I2C, SPI, UART, analog, digital, PWM, battery, deep sleep, 802.15.4.
---

# XIAO MG24 Sense — TinyGo Development Guide

Provides comprehensive reference for developing TinyGo firmware for the Seeed Studio XIAO MG24 Sense.

## When to Use

- Writing TinyGo firmware targeting the XIAO MG24 Sense
...
- Working with the onboard IMU or analog microphone on the MG24 Sense

## When NOT to Use

- For Arduino/C++ development on the XIAO MG24 Sense → use the `XIAO-MG24-Sense-Arduino` skill
- For the XIAO MG24 (without sensors) → use the `XIAO-MG24-TinyGo` skill
...
4. Driver support for the onboard IMU and microphone

### Current Alternatives

- Use **Arduino IDE** with the Silicon Labs Arduino core (see `XIAO-MG24-Sense-Arduino` skill)
...
uart.Write([]byte("Hello from XIAO MG24 Sense\r\n"))
```
...
The XIAO MG24 Sense supports Zigbee, Thread, Matter, and BLE 5.3 at the hardware level. These protocols require the Silicon Labs Gecko SDK and are not expected to be available through TinyGo in the near term.
...
```go
// Deep sleep is NOT recommended on the XIAO MG24 Sense due to bricking risk.
...
The XIAO MG24 Sense includes an onboard analog microphone for audio capture and sound detection:

```go
// When TinyGo support is available:
// The analog microphone output can be read via ADC
// Refer to the board package for the microphone ADC pin mapping
// mic := machine.ADC{Pin: machine.MIC_PIN}
...
The Sense variant includes a 6-axis IMU (accelerometer + gyroscope) connected via I2C:

```go
// When TinyGo support is available:
// The IMU is connected to the I2C bus
// Use a compatible TinyGo driver (e.g., from tinygo.org/x/drivers)
// i2c := machine.I2C0
// i2c.Configure(machine.I2CConfig{...})
// imu := imu_driver.New(i2c)
// imu.Configure(imu_driver.Config{...})
```

### AI/ML Hardware Accelerator (MVP)

The EFR32MG24 includes a Matrix Vector Processor (MVP) for on-device AI/ML inference — useful for processing IMU and microphone data on-device.

### All 22 GPIOs Support PWM

Every GPIO pin supports PWM output, providing maximum flexibility.
...
2. **Deep sleep bricks the board** — EM4 deep sleep makes the board unresponsive; use EM2 instead; double-tap reset to recover
...
5. **Same pinout as MG24** — The Sense variant has identical pin mappings; only adds onboard sensors
6. **IMU on I2C bus** — The onboard IMU uses the I2C bus; be aware of address conflicts with external I2C devices
7. **Analog microphone** — The microphone output is analog; read via ADC
8. **All pins are analog** — 19 of 22 GPIO pins support ADC (12-bit)
9. **All pins support PWM** — All 22 GPIO pins can output PWM
